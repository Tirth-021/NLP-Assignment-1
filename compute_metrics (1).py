...
# This file is a modified version of https://github.com/for-ai/language-confusion/blob/main/compute_metrics.py. Line 111 was added.
...
def compute_metrics(completions: Iterable[str], lang: str) -> dict[str, float]:
    ...
    for completion in completions:
        ...
        indices = [
            i for i, tokens in enumerate(line_tokens) if len(tokens) >= 5
        ]
        ...
        if lines:
            ...
            line_acc.append("too short")

    metrics = {}
    metrics["per_compl_acc"] = [l if l != "too short" else 1 for l in line_acc]
    line_acc = [l for l in line_acc if l != "too short"]
    ...
    if lang in (
        ...
        "ru",
        "zh",
    ):  # WPR is inaccurate for latin-script languages
        metrics["wpr"] = 1 - with_word_errors / max(
            1, non_skipped - with_line_errors
        )
    return metrics
    ...
    def average_metrics(metrics: list[dict]):
        ...
        for key in ("acc", "lpr", "wpr"):
            values = [
                ...
                if metrics_.get(key)
                is not None  # WPR can be missing for some languages
            ]
            if values:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute WPR and LPR over the model completions in given CSV file. "
        "Example: `python compute_metrics.py outputs/command-r-plus.csv`"
    )
    parser.add_argument(
        "csv_file",
        help="CSV file with the same format as the provided command-r outputs "
